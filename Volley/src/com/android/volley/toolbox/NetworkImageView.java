...
public class NetworkImageView extends ImageView {
	...
	public void setImageUrl(String url, ImageLoader imageLoader, int size) {
		mUrl = url;
		mImageLoader = imageLoader;
		// The URL has potentially changed. See if we need to load it.
		loadImageIfNecessary(false, size);
	}
					...
					public void onResponse(final ImageContainer response,
						...
						if (response.getBitmap() != null) {
							...
							if (isRounded) {
								...
								InputStream stream;
								try {
									stream = new URL(requestUrl).openStream();
									byte[] bytes = getBytes(stream);
									// 这3句是处理图片溢出的begin( 如果不需要处理溢出直接
									// opts.inSampleSize=1;)
