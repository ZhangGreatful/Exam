...
//                Toast.makeText(getContext(), getResources().getString(R.string.no_url), Toast.LENGTH_SHORT).show();
   ...
   private  int nowTime=0;
    public  static TimerCallBack callBack;  //在宽屏之后会出现对象消失，所以设置为static类型
    public  void setTimeCallBack(TimerCallBack callBack){
    ...
    public void setProgressAndTime(int progress, int secProgress, int currentTime, int totalTime) {
        ...
        //添加时间的回调 每秒返回一次当前时间和进度  progress>=98 设置为结束 调取照相
            ...
            if(currentTime-nowTime>=950||currentTime==0){
                Log.e("isplaying","yes");
                if(callBack!=null){
                    Log.e("isplaying","havecallback yes");
                    callBack.timeCallback(currentTime,progress);
                }else{
